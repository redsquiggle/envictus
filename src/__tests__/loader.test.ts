...
describe("loadConfig", () => {
	it("loads a TypeScript config file with default export", async () => {
		const configPath = resolve(import.meta.dirname, "../../examples/zod/env.config.ts");
	...
	it("loads a config with custom discriminator", async () => {
		const configPath = resolve(import.meta.dirname, "../../examples/custom-discriminator/env.config.ts");
		const config = await loadConfig(configPath);
	...
	it("loads a Valibot config file", async () => {
		const configPath = resolve(import.meta.dirname, "../../examples/valibot/env.config.ts");
		const config = await loadConfig(configPath);
	...
	it("loads an ArkType config file", async () => {
		const configPath = resolve(import.meta.dirname, "../../examples/arktype/env.config.ts");
		const config = await loadConfig(configPath);
	...
	it("loads a Yup config file", async () => {
		const configPath = resolve(import.meta.dirname, "../../examples/yup/env.config.ts");
		const config = await loadConfig(configPath);
	...
	it("loads a Joi config file", async () => {
		const configPath = resolve(import.meta.dirname, "../../examples/joi/env.config.ts");
		const config = await loadConfig(configPath);
